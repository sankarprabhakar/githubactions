...
## Job matrix configuration
    ...
    * strategy -> matrix need to be used
## Actions types
    * docker  ( only on Linux - Runs on docker container)
    * composite ( helps to combine multiple steps to one)
    * Javascript ( run son host runner)
    * action require meta data file to define the input, output and main entry point.

## Input and output to actions:
    *   inputs: command captures the input fields required by actions
    *   outpus: command captures the output that will be generated by actions
    *   using: command captures if the action is JS, Docker or composite
    ```
     using: node20
     main: `main.js`
    ```
